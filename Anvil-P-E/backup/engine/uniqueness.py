...
class EscrowLog:
    def __init__(self):
        # dict[(table, col, val) -> list of (peer_id, row_pk)]
        self.claims: dict[tuple[str, str, Any], list[tuple[str, Any]]] = {}

    def claim(self, table: str, col: str, val: Any, peer_id: str, row_pk: Any) -> None:
        key = (table, col, val)
    ...
    def resolve_all(self, store) -> None:
        # Collect final status decisions first (pk -> status), then apply.
        # This avoids iteration-order sensitivity from direct mutation.
        pending_updates: dict[tuple, str] = {}  # (table, pk) -> status

        for (table, col, val), claimants in self.claims.items():
            if not claimants:
                continue

            # Sort claimants deterministically by (peer_id, str(row_pk))
            # so both peers compute winner identically regardless of insertion order.
            sorted_claimants = sorted(claimants, key=lambda c: (str(c[0]), str(c[1])))

            if len(sorted_claimants) == 1:
                peer_id, row_pk = sorted_claimants[0]
                # Only upgrade to committed, never downgrade an already-rejected row
                key = (table, row_pk)
