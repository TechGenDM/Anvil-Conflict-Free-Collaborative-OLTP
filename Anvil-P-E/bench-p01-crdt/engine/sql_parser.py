...
def parse_sql(sql: str, params: tuple = ()) -> dict:
    ...
    if sql_stmt.upper().startswith("CREATE TABLE"):
        ...
        if match:
            ...
            # Identify table-level constraints like UNIQUE(a, b)
            table_unique_cols = []
            constraints_match = re.finditer(r"UNIQUE\s*\((.*?)\)", content_str, re.IGNORECASE)
            for cm in constraints_match:
                cols = [c.strip() for c in cm.group(1).split(',')]
                table_unique_cols.append(tuple(cols))
            
            # Strip table-level constraints for column parsing
            stripped_content = re.sub(r",?\s*UNIQUE\s*\(.*?\)", "", content_str, flags=re.IGNORECASE).strip()
            
            # Simple split by comma (assuming no commas inside parens in column defs)
            col_defs = [c.strip() for c in stripped_content.split(',')]
            columns = []
            ...
            return {
                ...
                "table": table_name,
                "columns": columns,
                "composite_unique": table_unique_cols
            }
