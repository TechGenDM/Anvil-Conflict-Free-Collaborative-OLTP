...
class CRDTStore:
    ...
    def snapshot_state(self) -> dict:
        ...
        for table in sorted(self.tables.keys()):
            ...
            # Combine all columns involved in unique constraints
            u_cols = set(schema_info.get("unique_cols", []))
            for comp in schema_info.get("composite_unique", []):
                u_cols.update(comp)
            unique_list = list(u_cols)
            
            # FK columns
            fk_list = list(schema_info.get("fk_cols", {}).keys())

            for pk in sorted_pks:
                row = table_rows[pk]
                if row.is_visible(self.fk_policy):
                    # Sort columns within each row for determinism
                    row_dict = row.snapshot(unique_list, fk_list, self.fk_policy)
